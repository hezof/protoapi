...
func (ctx *Context) Scheme(dst interface{}, tag string) error {
	...
	if dstTyp == nil || dstTyp.Kind() != reflect.Ptr {
		return core.ErrInvalidStructPointer
	}

	dstVal := reflect.ValueOf(dst)
	if dstVal.IsNil() {
		return core.ErrInvalidMemoryOrNilPointer
	}
...
func adaptStruct(vs *values, dstTyp reflect.Type, dstVal *reflect.Value, tag string) (set bool, err error) {
	 ...
	 *************************************/
__NEXT__:
	for _, fld := range core.CacheStructField(dstTyp) {
		fldKey := fld.Tags[tag]
		if fldKey == "-" {
			continue __NEXT__
		}
		...
		if ss, ok := vs.form[fldKey]; ok {
			if fldTyp.Kind() == reflect.Slice && fldTyp != core.RTypeBytes {
...
func adaptValue(org string, dstTyp reflect.Type, dstVal *reflect.Value) error {
	...
	// 优化几种常见特殊的类型
	if dstTyp == core.RTypeDuration {
		if x, err := core.AsDuration(org); err != nil {
			return err
		} else {
			dstVal.SetInt(int64(x))
			return nil
		}
	} else if dstTyp == core.RTypeTime {
		if x, err := core.AsTime(org); err != nil {
			return err
		} else {
			dstVal.Set(reflect.ValueOf(x))
			return nil
		}
	} else if dstTyp == core.RTypeBytes {
		if x, err := core.AsBytes(org); err != nil {
			return err
		} else {
			dstVal.Set(reflect.ValueOf(x))
			return nil
	...
	case reflect.String:
		...
		return nil
	case reflect.Bool:
		if x, err := core.AsBool(org); err != nil {
			return err
		} else {
			dstVal.SetBool(x)
			return nil
	...
	case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
		if x, err := core.AsInt64(org); err != nil {
			return err
		} else {
			dstVal.SetInt(x)
			return nil
	...
	case reflect.Uint, reflect.Uint8, reflect.Uint16, reflect.Uint32, reflect.Uint64:
		if x, err := core.AsUint64(org); err != nil {
			return err
		} else {
			dstVal.SetUint(x)
			return nil
	...
	case reflect.Uintptr:
		if x, err := core.AsUintptr(org); err != nil {
			return err
		} else {
			dstVal.Set(reflect.ValueOf(x))
			return nil
	...
	case reflect.Float32, reflect.Float64:
		if x, err := core.AsFloat64(org); err != nil {
			return err
		} else {
			dstVal.SetFloat(x)
			return nil
	...
	case reflect.Complex64, reflect.Complex128:
		if x, err := core.AsComplex128(org); err != nil {
			return err
		} else {
			dstVal.SetComplex(x)
			return nil
