...
func DecodeRequest(in io.Reader, req any) error {
	err := protojson.DecodeProtoJsonReader(in, req)
	if err != nil {
		return framework.StatusError(http.StatusBadRequest, profile.DefaultBadRequestErrorCode, err.Error())
	}
	return nil
}
...
func ParamBool(f func(string) (string, error), key string, ptr *bool) error {
	val, err := f(key)
	if err != nil {
		return framework.StatusError(profile.DefaultBadRequestErrorStatus, profile.DefaultBadRequestErrorCode, fmt.Sprintf("[%v] %v", key, err))
	} else if val == "" {
		return nil
	}
	*ptr = val == "true"
