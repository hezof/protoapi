...
type Config struct {
	...
	HttpReadTimeout     time.Duration `json:"http_read_timeout"`        // http ReadTimeout is the maximum duration for reading the entire request, including the body
	HttpWriteTimeout    time.Duration `json:"http_read_header_timeout"` // http WriteTimeout is the maximum duration before timing out writes of the response
	HttpIdleTimeout     time.Duration `json:"http_idle_timeout"`        // http IdleTimeout is the maximum amount of time to wait for the next request when keep-alives are enabled.
