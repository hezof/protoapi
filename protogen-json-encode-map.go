...
/*
	需要支持string map(键为string的map)的编码
*/
...
func EncodeMessageMap[Message any](w *JsonEncoder, value map[string]*Message, f func(w *JsonEncoder, m *Message)) {
	switch {
	case value == nil:
		w.ensure(4)
		w.buff = append(w.buff, 'n', 'u', 'l', 'l')
...
func EncodeMessageMap_OmitEmpty[Message any](w *JsonEncoder, name string, value map[string]*Message, f func(w *JsonEncoder, m *Message)) {
	if len(value) != 0 {
		w.ensure(4 + len(name))
		w.buff = append(w.buff, quotes)
		w.buff = append(w.buff, name...)
...
func EncodeMessageMap_WithEmpty[Message any](w *JsonEncoder, name string, value map[string]*Message, f func(w *JsonEncoder, m *Message)) {
	switch {
	case value == nil:
		w.ensure(8 + len(name))
		w.buff = append(w.buff, quotes)
...
func EncodeMessageMap_ConvEmpty[Message any](w *JsonEncoder, name string, value map[string]*Message, f func(w *JsonEncoder, m *Message)) {
	switch {
	case value == nil || len(value) == 0:
		w.ensure(6 + len(name))
		w.buff = append(w.buff, quotes)
